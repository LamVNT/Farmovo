-- Sample Data for Farmovo Project (Corrected Version)
-- This script creates 5 rows of sample data for each model
-- Make sure to run this AFTER your Spring Boot application has created the tables

-- First, let's check if tables exist and clear existing data
DO $$
BEGIN
    -- Clear existing data in reverse dependency order
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'debt_notes') THEN
        DELETE FROM debt_notes;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stocktake_details') THEN
        DELETE FROM stocktake_details;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stocktakes') THEN
        DELETE FROM stocktakes;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'import_transaction_details') THEN
        DELETE FROM import_transaction_details;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'import_transactions') THEN
        DELETE FROM import_transactions;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sale_transactions') THEN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_authorities') THEN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'authorities') THEN
        DELETE FROM authorities;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'forgot_password') THEN
        DELETE FROM forgot_password;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'change_status_logs') THEN
        DELETE FROM change_status_logs;
    END IF;
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'authorities') THEN
        ALTER SEQUENCE IF EXISTS authorities_id_seq RESTART WITH 1;
    END IF;
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'zones') THEN
        ALTER SEQUENCE IF EXISTS zones_id_seq RESTART WITH 1;
    END IF;
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'import_transactions') THEN
        ALTER SEQUENCE IF EXISTS import_transactions_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'import_transaction_details') THEN
        ALTER SEQUENCE IF EXISTS import_transaction_details_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sale_transactions') THEN
        ALTER SEQUENCE IF EXISTS sale_transactions_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stocktakes') THEN
        ALTER SEQUENCE IF EXISTS stocktakes_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'debt_notes') THEN
        ALTER SEQUENCE IF EXISTS debt_notes_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'change_status_logs') THEN
        ALTER SEQUENCE IF EXISTS change_status_logs_id_seq RESTART WITH 1;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'forgot_password') THEN
        ALTER SEQUENCE IF EXISTS forgot_password_id_seq RESTART WITH 1;
    END IF;
END $$;
...
-- 10. IMPORT TRANSACTION DETAILS (with correct column names)
INSERT INTO import_transaction_details (import_quantity, remain_quantity, expire_date, unit_import_price, unit_sale_price, zones_id, is_check, name, product_id, import_transaction_id, created_at, updated_at) VALUES
(50, 45, '2024-02-15 00:00:00', 20000.00, 25000.00, '1,2', true, 'IMP001', 1, 1, NOW(), NOW()),
(25, 23, '2024-02-16 00:00:00', 40000.00, 50000.00, '1', true, 'IMP002', 2, 1, NOW(), NOW()),
(20, 18, '2024-02-17 00:00:00', 60000.00, 75000.00, '2', true, 'IMP003', 3, 2, NOW(), NOW()),
(40, 38, '2024-02-18 00:00:00', 25000.00, 30000.00, '3', true, 'IMP004', 4, 3, NOW(), NOW()),
(30, 28, '2024-02-19 00:00:00', 30000.00, 35000.00, '1', true, 'IMP005', 5, 4, NOW(), NOW());
...
-- 13. STOCKTAKE DETAILS (Note: This is stored as JSON in the detail column)
-- The stocktake details are stored as JSON in the 'detail' column of stocktakes table
-- This is handled by the application logic, not direct SQL inserts
