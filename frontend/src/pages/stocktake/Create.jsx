...
const CreateStocktakePage = () => {
  ...
  return (
    <Box sx={{ maxWidth: 900, margin: "40px auto", background: "#fff", p: 4, borderRadius: 3, boxShadow: 2 }}>
            ...
            {rows.map((row, idx) => (
              <TableRow key={idx} hover>
                ...
                <TableCell sx={{ minWidth: 160 }}>
                  <Button variant="outlined" size="small" onClick={() => handleOpenZoneDialog(idx)} sx={{ minWidth: 120 }}>
                    {row.zones.length > 0 ? (
                      (() => {
                        const z = zones.find(z => String(z.id) === String(row.zones[0]));
                        return z ? <Chip label={z.zoneName} size="small" sx={{ mr: 0.5, mb: 0.5 }} /> : null;
                      })()
                    ) : "Chọn khu vực"}
                  </Button>
                </TableCell>
      ...
      <Button onClick={handleAddRow} variant="outlined" startIcon={<AddIcon />} sx={{ mb: 2, borderRadius: 2, fontWeight: 600 }}>
        Thêm dòng
      </Button>
      <Box sx={{ mt: 3, display: "flex", justifyContent: "flex-end", gap: 2 }}>
        <Button onClick={handleCancel}>Hủy</Button>
        <Button variant="contained" onClick={handleSubmit} sx={{ fontWeight: 600, borderRadius: 2 }}>Xác nhận</Button>
      </Box>
      <ZoneCheckboxDialog
        open={zoneDialogOpen}
