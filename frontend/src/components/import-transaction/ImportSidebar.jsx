...
const ImportSidebar = ({
    ...
    return (
            ...
            <div>
                ...
                <div className="relative">
                    <TextField
                        ...
                        onBlur={() => {
                            // Delay để cho phép hover hoạt động
                            setTimeout(() => {
                                if (!hoveredSupplier) {
                    ...
                    {(supplierDropdownOpen || supplierSearch.trim() !== '') && filteredSuppliers.length > 0 && (
                        <div 
                            ...
                            onMouseLeave={() => {
                                ...
                                setTimeout(() => setSupplierDropdownOpen(false), 100);
                            }}
                        >
        ...
        <Popover
            open={Boolean(hoveredSupplier) && Boolean(hoverAnchorEl) && (supplierDropdownOpen || supplierSearch.trim() !== '')}
                ...
                <div className="p-4 bg-white">
                    <div className="space-y-3">
                        ...
                        <div className="border-b border-gray-100 pb-2">
                            <h3 className="text-lg font-bold text-gray-800">{hoveredSupplier.name}</h3>
                            <div className="text-xs text-blue-600 font-medium">Nhà cung cấp</div>
                        </div>
                        ...
                        <div className="space-y-2">
                            {hoveredSupplier.phone && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-gray-600">📞 {hoveredSupplier.phone}</span>
                                </div>
                            )}
                            ...
                            {hoveredSupplier.email && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-gray-600">✉️ {hoveredSupplier.email}</span>
                                </div>
                            )}
                            ...
                            {hoveredSupplier.address && (
                                <div className="flex items-start gap-2">
                                    ...
                                    <span className="text-sm text-gray-600">📍 {hoveredSupplier.address}</span>
                                </div>
                            )}
                        </div>
                        ...
                        {hoveredSupplier.totalDebt !== undefined && (
                            <div className="pt-2 border-t border-gray-100">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className={`text-sm font-bold ${hoveredSupplier.totalDebt > 0 ? 'text-red-600' : 'text-green-600'}`}>
                        ...
                        <div className="pt-2 border-t border-gray-100">
                            <div className="text-xs text-gray-500 space-y-1">
                                {hoveredSupplier.createAt && (
                                    <div>Ngày tạo: {new Date(hoveredSupplier.createAt).toLocaleDateString('vi-VN')}</div>
                                )}
                                {hoveredSupplier.createBy && (
                                    <div>Người tạo: {hoveredSupplier.createBy}</div>
                                )}
                            </div>
                        </div>
                    </div>
